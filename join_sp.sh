...
while IFS=':' read -r line_number _; do
	...
	for each in $(ls -1 $4/nmode$MODENEUTRO/geom/*.log | sort -n -t _ -k 2 | xargs -n 1 basename); do

		# Find and scale the neutral energy
		ENEUTRAL=$(grep "SCF Done:" $4/nmode$MODENEUTRO/geom/$each | awk '{print $5}')
		FENEUTRAL=$(awk -v s1=$EPAR -v s2=$ENEUTRAL -v s3=$hf2ev 'BEGIN { printf "%.3f\n", (- s1 + s2) * s3}')
	
		# Find and scale the anion energy
		EANION=$(grep "SCF Done:" $5/nmode$MODEANION/geom/$each | awk '{print $5}')
		FEANION=$(awk -v s1=$EPAR -v s2=$EANION -v s3=$hf2ev 'BEGIN { printf "%.3f\n", (- s1 + s2) * s3}')

		# Find and scale the first anion excited state
		EXANION=$(grep "Total Energy, E(TD-HF/TD-DFT) =" $5/nmode$MODEANION/geom/$each | awk '{print $5}')
    	...
    	if (( $(echo "$FENEUTRAL > 1000" | bc -l) )) || (( $(echo "$FEANION > 1000" | bc -l) )); then
        	echo "	Some calculation in $FACTOR step does not converged. Check gaussian .log neutral and anion filesi for $MNUMBER normal mode." 
